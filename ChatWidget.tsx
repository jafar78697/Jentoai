...
const ChatWidget: React.FC = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: 'model', text: 'Jento AI Architect v2.0 active. Connection established with live production core. How can I assist?', timestamp: new Date() }
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await fetch('https://n8n.jentoaiautomation.online/webhook/a3b709ac-031d-423c-96a1-55cf1facf012/chat', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        // Robust extraction logic for n8n/AI responses
        if (typeof data === 'string') {
          aiResponse = data;
        } else if (Array.isArray(data) && data.length > 0) {
          const first = data[0];
          aiResponse = first.output || first.response || first.text || JSON.stringify(first);
        } else {
          aiResponse = data.output || data.response || data.text || data.message || JSON.stringify(data);
        }
      ...
      setMessages(prev => [...prev, {
        role: 'model',
        text: "CRITICAL: CONNECTION BLOCKED. The Architect Core is unable to reach the production sync node. Please check your network or try again later.",
        timestamp: new Date()
      }]);
    } finally {
      setIsTyping(false);
            ...
            <div className="flex items-center space-x-4 relative z-10">
              ...
              <div>
                ...
                <div className="flex items-center space-x-2 mt-0.5">
                  <span className={`w-2 h-2 rounded-full animate-pulse ${connectionStatus === 'error' ? 'bg-red-400' : 'bg-green-400'
                    }`}></span>
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 space-y-6 bg-slate-950/40">
            {messages.map((msg, idx) => (
              <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] p-4 rounded-2xl text-[13px] font-medium leading-relaxed whitespace-pre-wrap ${msg.role === 'user'
